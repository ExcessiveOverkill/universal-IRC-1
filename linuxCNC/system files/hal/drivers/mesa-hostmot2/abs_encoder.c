...
int hm2_absenc_setup_fabs(hostmot2_t *hm2, hm2_sserial_remote_t *chan, 
    ...
    if (hal_param_bit_newf(HAL_RW, &(chan->params->bit_param),
            hm2->llio->comp_id,"%s.single-pair",
            chan->name)){
        HM2_ERR("error adding param fanuc bit param, aborting\n");
        return -EINVAL;
    }
...
int hm2_absenc_parse_format(hm2_sserial_remote_t *chan,  hm2_absenc_format_t *def){
    ...
    char* A64i = "%10p%11plow%16l%3pencoder%16h%3pcomm%8u%6p";
    ...
    if (chan->myinst == HM2_GTAG_FABS && strncmp(format, "A64i",4) == 0){
        format = A64i;
    }
...
void hm2_absenc_write(hostmot2_t *hm2){
    ...
    for (i = 0; i < hm2->absenc.num_chans; i ++) {
        ...
        case HM2_GTAG_FABS:
			...
			//if (chan->params->bit_param){
			//	chan->params->u32_param = 6;
			//	chan->params->float_param = 2721.088;
			//	}
        
            if (chan->params->timer_num > 4) chan->params->timer_num = 4;
            //if (chan->params->u32_param > 0xF) chan->params->u32_param = 0xF;
            dds = (rtapi_u32)((0x100000 * ((chan->params->float_param * 1000)
            / hm2->absenc.clock_frequency)));
            filt = (hm2->absenc.clock_frequency/(chan->params->float_param * 1000)) / 2;
