...
void AsciiProtocol::process_line(cbufptr_t buffer) {
    ...
    switch(cmd[0]) {
        ...
        case 'z': cmd_linuxcnc_fanuc(cmd, use_checksum);              break;  // Custom cmd(set encoder and torque from linuxcnc)
        default : cmd_unknown(nullptr, use_checksum);                 break;
    }
}
// @brief Executes custom command: set encoder pos and motor torque
...
void AsciiProtocol::cmd_linuxcnc_fanuc(char * pStr, bool use_checksum) {
    char enable_0, enable_1;
    unsigned encoder_0, encoder_1;
    float torque_0, torque_1;
    bool error = 0;
    Axis& axis0 = axes[0];
    Axis& axis1 = axes[1];
    int numscan = sscanf(pStr, "z %u %f %c %u %f %c", &encoder_0, &torque_0, &enable_0, &encoder_1, &torque_1, &enable_1); 
    if(numscan < 0){
        error = 1;
    }

    if(enable_0 == '2'){
        if(axis0.current_state_ != 8){
            axis0.requested_state_ = Axis::AXIS_STATE_CLOSED_LOOP_CONTROL;
        }
    }
    else if(axis0.error_ != 0){
        error = 1;
    }
    else if(enable_0 == '1'){
        axis0.encoder_.count_in_cpr_ = encoder_0;
        axis0.controller_.input_torque_ = torque_0;
        if(axis0.current_state_ != 8){
            error = 1;
        }
    }
    else if(enable_0 == '0'){
        axis0.requested_state_ = Axis::AXIS_STATE_IDLE;
        axis0.controller_.input_torque_ = 0;
    }
    else{
        error = 1;
    }
    ...
    if(enable_1 == '2'){
        if(axis1.current_state_ != 8){
            axis1.requested_state_ = Axis::AXIS_STATE_CLOSED_LOOP_CONTROL;
        }
    }
    else if(axis1.error_ != 0){
        error = 1;
    }
    else if(enable_1 == '1'){
        axis1.encoder_.count_in_cpr_ = encoder_1;
        axis1.controller_.input_torque_ = torque_1;
        if(axis1.current_state_ != 8){
            error = 1;
        }
    }
    else if(enable_1 == '0'){
        axis1.requested_state_ = Axis::AXIS_STATE_IDLE;
        axis1.controller_.input_torque_ = 0;
    }
    else{
        error = 1;
    }
    ...
    char isEnabled_0 = (axis0.current_state_ == 8) ? '1' : '0';
    char isEnabled_1 = (axis1.current_state_ == 8) ? '1' : '0';
    float temperature = (axis0.motor_.fet_thermistor_.temperature_ > axis1.motor_.fet_thermistor_.temperature_) ? axis0.motor_.fet_thermistor_.temperature_ : axis1.motor_.fet_thermistor_.temperature_;
    //respond(use_checksum, "%c %c %c %04.1f %04.1f %04.1f", status_bit, isEnabled_0, isEnabled_1, vbus_voltage, ibus_, temperature);    //send data back: status, bus voltage, bus current, temp
    respond(use_checksum, "%c %c %c %04.1f", status_bit, isEnabled_0, isEnabled_1, temperature);
